...
def main(model, terminal_prompt, voice_mode=False, verbose_mode=False):
    ...
    if terminal_prompt:  # Skip objective prompt if it was given as an argument
        ...
        print(
            f"{ANSI_GREEN}[Vity]{ANSI_RESET} Listening for your command... (speak now)"
        )
        try:
        ...
        print(
            f"[{ANSI_GREEN}Vity {ANSI_RESET}|{ANSI_BRIGHT_MAGENTA} {model}{ANSI_RESET}]\n{USER_QUESTION}"
        )
    ...
    while True:
        if config.verbose:
            print("[Vity] loop_count", loop_count)
        try:
            ...
            print(
                f"{ANSI_GREEN}[Vity]{ANSI_RED}[Error] -> {e} {ANSI_RESET}"
            )
            break
        except Exception as e:
            print(
...
def operate(operations, model):
    ...
    for operation in operations:
        if config.verbose:
            print("[Vity][operate] operation", operation)
        # wait one second
        time.sleep(1)
        ...
        operate_detail = ""
        if config.verbose:
            print("[Vity][operate] operate_type", operate_type)
        ...
        if operate_type == "press" or operate_type == "hotkey":
            ...
            print(
                f"{ANSI_GREEN}[Vity]{ANSI_RED}[Error] unknown operation response :({ANSI_RESET}"
            )
            print(
                f"{ANSI_GREEN}[Vity]{ANSI_RED}[Error] AI response {ANSI_RESET}{operation}"
            )
            return True
