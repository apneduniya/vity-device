...
async def get_next_action(model, messages, objective, session_id):
    if config.verbose:
        print("[Vity][get_next_action]")
        print("[Vity][get_next_action] model", model)
...
def call_gpt_4o(messages):
    ...
    try:
        ...
        print(
            f"{ANSI_GREEN}[Vity]{ANSI_BRIGHT_MAGENTA}[Operate] That did not work. Trying again {ANSI_RESET}",
            e,
        )
        print(
            f"{ANSI_GREEN}[Vity]{ANSI_RED}[Error] AI response was {ANSI_RESET}",
            content,
        )
        if config.verbose:
            traceback.print_exc()
...
def call_gemini_pro_vision(messages, objective):
    """
    Get the next action for Vity using Gemini Pro Vision
    """
    if config.verbose:
        print(
            "[Vity][call_gemini_pro_vision]",
        )
    # sleep for a second
    time.sleep(1)
    try:
        ...
        print(
            f"{ANSI_GREEN}[Vity]{ANSI_BRIGHT_MAGENTA}[Operate] That did not work. Trying another method {ANSI_RESET}"
        )
        if config.verbose:
            print("[Vity][Operate] error", e)
            traceback.print_exc()
        return call_gpt_4o(messages)
...
async def call_gpt_4o_with_ocr(messages, objective, model):
    ...
    try:
        ...
        print(
            f"{ANSI_GREEN}[Vity]{ANSI_BRIGHT_MAGENTA}[{model}] That did not work. Trying another method {ANSI_RESET}"
        )
        if config.verbose:
            print("[Vity][Operate] error", e)
            traceback.print_exc()
    ...
    try:
        ...
        for operation in content:
            ...
            if operation.get("operation") == "click":
                ...
                if config.verbose:
                    print(
                        "[Vity][call_gpt_4_vision_preview_labeled] label",
                        label,
                    )
                ...
                if config.verbose:
                    print(
                        "[Vity][call_gpt_4_vision_preview_labeled] coordinates",
                        coordinates,
                    )
                image = Image.open(
                ...
                if config.verbose:
                    print(
                        "[Vity][call_gpt_4_vision_preview_labeled] click_position_percent",
                        click_position_percent,
                    )
                if not click_position_percent:
                    print(
                        f"{ANSI_GREEN}[Vity]{ANSI_RED}[Error] Failed to get click position in percent. Trying another method {ANSI_RESET}"
                    )
                    return call_gpt_4o(messages)
                ...
                if config.verbose:
                    print(
                        "[Vity][call_gpt_4_vision_preview_labeled] new click operation",
                        operation,
                    )
                processed_content.append(operation)
            else:
                if config.verbose:
                    print(
                        "[Vity][call_gpt_4_vision_preview_labeled] .append none click operation",
                        operation,
                    )
            ...
            if config.verbose:
                print(
                    "[Vity][call_gpt_4_vision_preview_labeled] new processed_content",
                    processed_content,
                )
            return processed_content
...
def call_ollama_llava(messages):
    ...
    try:
        ...
        print(
            f"{ANSI_GREEN}[Vity]{ANSI_RED}[Operate] Couldn't connect to Ollama. With Ollama installed, run `ollama pull llava` then `ollama serve`{ANSI_RESET}",
            e,
        )

    except Exception as e:
        print(
            f"{ANSI_GREEN}[Vity]{ANSI_BRIGHT_MAGENTA}[llava] That did not work. Trying again {ANSI_RESET}",
            e,
        )
        print(
            f"{ANSI_GREEN}[Vity]{ANSI_RED}[Error] AI response was {ANSI_RESET}",
...
async def call_claude_3_with_ocr(messages, objective, model):
    ...
    try:
        ...
        # rework for json mode output
            ...
            if config.verbose:
                print(
                    f"{ANSI_GREEN}[Vity]{ANSI_RED}[Error] JSONDecodeError: {e} {ANSI_RESET}"
                )
            response = client.messages.create(
                model="claude-3-opus-20240229",
                max_tokens=3000,
        ...
        if config.verbose:
            print(
                f"{ANSI_GREEN}[Vity]{ANSI_BRIGHT_MAGENTA}[{model}] content: {content} {ANSI_RESET}"
            )
        processed_content = []
